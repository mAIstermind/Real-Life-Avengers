...
const THEMES: { id: DashboardTheme, label: string, icon: string }[] = [
    { id: 'noir', label: 'Noir Script', icon: '📽️' },
...
export const Setup: React.FC<SetupProps> = (props) => {
    ...
    const themeConfig = {
        noir: {
            ...
            section: "bg-black/60 border border-white/10 p-8",
            input: "bg-zinc-50 border-4 border-black text-black font-serif focus:bg-white focus:ring-4 focus:ring-red-600 transition-all text-2xl italic px-4 py-2 placeholder-zinc-400 rounded-none",
            button: "bg-red-600 text-white font-comic hover:bg-white hover:text-black transition-all py-8 border-t-4 border-black",
            accent: "text-red-600",
            label: "text-white/80 uppercase tracking-[0.4em] font-mono text-[11px] mb-4 bg-black/40 px-2 py-1 inline-block"
        },
        pixar: {
            ...
            input: "bg-white border-4 border-blue-400 rounded-2xl font-sans font-black text-blue-600 focus:ring-4 focus:ring-orange-200 transition-all px-6 py-4 placeholder-blue-100 shadow-inner",
        ...
        cyber: {
            ...
            section: "bg-cyan-950/40 border border-cyan-500/30 p-8 rounded-lg",
            input: "bg-[#0a1a2f] border-2 border-cyan-400 text-cyan-400 font-mono focus:border-white focus:shadow-[0_0_20px_rgba(34,211,238,0.5)] px-4 py-3 placeholder-cyan-900",
            ...
            label: "text-cyan-400 font-mono text-xs uppercase mb-3 flex items-center gap-2"
        },
        western: {
            container: "bg-[#ebd9a9] border-[6px] border-[#3e2723] shadow-[20px_20px_0_#3e2723] p-16 rounded-sm",
            section: "bg-[#dcc7a1] border-4 border-[#3e2723] p-10 shadow-inner",
            input: "bg-[#fdf5e6] border-2 border-[#3e2723] font-serif italic text-3xl text-[#2c1810] focus:bg-white transition-all px-4 py-3 placeholder-[#3e2723]/30 shadow-[inset_4px_4px_0_rgba(0,0,0,0.1)]",
            button: "bg-[#3e2723] text-[#ebd9a9] font-serif font-black text-5xl py-8 hover:bg-red-900 transition-colors",
            accent: "text-[#3e2723]",
            label: "text-[#3e2723] font-serif font-black uppercase tracking-[0.3em] text-xs mb-3"
        }
    }[theme];
    ...
    const brainstormNemesis = async () => {
        ...
        } catch (e) { console.error(e); } finally { setIsBrainstorming(true); }
    };
                    ...
                    <div className={`flex flex-col gap-10 ${themeConfig.section}`}>
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-6 mb-10">
                                <div>
                                    <p className="text-[10px] font-bold uppercase opacity-80 mb-2">Universe</p>
                                    <select value={props.selectedGenre} onChange={(e) => props.onGenreChange(e.target.value)} className={`w-full bg-white text-black border-b-2 border-current focus:outline-none font-bold text-xs py-3 px-2 cursor-pointer rounded-t`}>
                                        {GENRES.map(g => <option key={g} value={g} className="text-black">{g}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <p className="text-[10px] font-bold uppercase opacity-80 mb-2">Length</p>
                                    <select value={props.storyLength} onChange={(e) => props.onStoryLengthChange(parseInt(e.target.value))} className={`w-full bg-white text-black border-b-2 border-current focus:outline-none font-bold text-xs py-3 px-2 cursor-pointer rounded-t`}>
                            ...
                            <div className="mb-10">
                                <p className="text-[10px] font-bold uppercase opacity-80 mb-2">Mission Objective</p>
                                ...
                                <button onClick={props.onFeelingLucky} className="mt-4 text-[11px] font-bold opacity-60 hover:opacity-100 uppercase italic tracking-[0.2em] transition-opacity flex items-center gap-2">
