...
export const Book: React.FC<BookProps> = (props) => {
    ...
    if (props.comicFaces.length > 0) {
        const sorted = [...props.comicFaces].sort((a, b) => (a.pageIndex || 0) - (b.pageIndex || 0));
        const maxPage = Math.max(...sorted.map(f => f.pageIndex || 0));
        sheetsToRender.push({ front: sorted.find(f => f.pageIndex === 0), back: sorted.find(f => f.pageIndex === 1) });
        for (let i = 2; i <= maxPage; i += 2) {
            sheetsToRender.push({ front: sorted.find(f => f.pageIndex === i), back: sorted.find(f => f.pageIndex === i + 1) });
        }
